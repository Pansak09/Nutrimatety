...
- อัปโหลดรูปอาหารและวิเคราะห์ภาพด้วยโมเดล YOLOv8 ที่ `fastapi_backend/models/best.pt` เพื่อช่วยระบุชนิดอาหาร
- จัดการเมนูอาหารสำหรับผู้ดูแลระบบผ่าน API และหน้า `/admin/menu-page`


- **ระบบคำนวณค่าสุขภาพและโภชนาการอัตโนมัติ** โดยประมวลผลจากข้อมูลโปรไฟล์ผู้ใช้ ดังนี้:
  - **อายุ (Age):** คำนวณจากวันเกิด (`date_of_birth`) และวันปัจจุบัน
  - **ดัชนีมวลกาย (BMI):** คำนวณจาก น้ำหนักปัจจุบัน (`current_weight`) และ ส่วนสูง (`height`)
  - **อัตราการเผาผลาญพลังงานพื้นฐาน (BMR):** ใช้สมการ Mifflin-St Jeor โดยอ้างอิงจาก เพศ (`gender`), น้ำหนัก (`current_weight`), ส่วนสูง (`height`) และอายุ
  - **พลังงานที่ใช้ต่อวัน (TDEE):** คำนวณจากการนำ BMR ไปคูณกับตัวคูณระดับกิจกรรม (`lifestyle` เช่น sedentary, light, moderate, active, athlete)
  - **เป้าหมายสารอาหาร (Macros & Target Calories):** คำนวณตามเป้าหมาย (`goal` เช่น ลดน้ำหนัก, เพิ่มน้ำหนัก, รักษาน้ำหนัก), น้ำหนักปัจจุบัน และ TDEE
    - **พลังงานเป้าหมาย (Target Calories):** ปรับลดหรือเพิ่มจาก TDEE ตามเป้าหมาย (ลดน้ำหนักใช้ 80% ของ TDEE, เพิ่มน้ำหนักใช้ 115% ของ TDEE, ปกติใช้ 100%)
    - **โปรตีน (Protein):** คำนวณสัดส่วนต่อน้ำหนักตัว (ลดน้ำหนักใช้ 2.0g/kg, เพิ่มน้ำหนัก 2.2g/kg, ปกติ 1.4g/kg)
    - **ไขมัน (Fat):** คำนวณเป็นสัดส่วนของพลังงานเป้าหมายรวม (ลดน้ำหนักใช้ 25%, แบบอื่น 30%)
    - **คาร์โบไฮเดรต (Carb):** นำพลังงานเป้าหมายที่เหลือจากโปรตีนและไขมัน มาคำนวณเป็นปริมาณคาร์โบไฮเดรต (กรัม)
...
| ฟิลด์ | ชนิดข้อมูล | เก็บข้อมูล | ใช้งาน |
| --- | --- | --- | --- |
| `id` | integer | รหัสผู้ใช้ที่ระบบสร้าง | เชื่อมผู้ใช้กับโปรไฟล์และมื้ออาหาร |
| `email` | string (email) | อีเมลสำหรับระบุตัวตน | สมัครสมาชิกและเข้าสู่ระบบ |
| `password` | string | รหัสผ่านที่ผู้ใช้กรอก | ส่งเฉพาะตอนสมัคร/ล็อกอิน; ระบบเก็บเป็น hash ไม่ส่งกลับ |
| `access_token` | string | JWT สำหรับยืนยันตัวตน | เก็บใน AsyncStorage แล้วส่งใน `Authorization` header |
| `token_type` | string | รูปแบบโทเค็น | ปัจจุบันคือ `bearer` |
| `signup_completed` | boolean | สถานะว่าผู้ใช้มีโปรไฟล์แล้วหรือยัง | ใช้กำหนดว่าจะพาไปสร้างโปรไฟล์หรือหน้าแอปหลัก |

### โปรไฟล์สุขภาพ

| ฟิลด์ | ชนิดข้อมูล | เก็บข้อมูล | ใช้งาน |
| --- | --- | --- | --- |
| `username` | string | ชื่อที่แสดงในแอป | แสดงหน้าโปรไฟล์; ต้องไม่ซ้ำ |
| `gender` | string | เพศของผู้ใช้ | ใช้คำนวณสุขภาพและจับกลุ่มสำหรับ analytics |
| `date_of_birth` | date (`YYYY-MM-DD`) | วันเกิด | คำนวณอายุและใช้จับกลุ่มเปรียบเทียบ |
| `height` | integer | ส่วนสูง (เซนติเมตร) | คำนวณ BMI, BMR และ TDEE |
| `current_weight` | integer | น้ำหนักปัจจุบัน (กิโลกรัม) | คำนวณ BMI, BMR และ TDEE |
| `target_weight` | integer | น้ำหนักเป้าหมาย (กิโลกรัม) | แสดงเป้าหมายสุขภาพ |
| `goal` | string | เป้าหมาย เช่น `ลดน้ำหนัก`, `เพิ่มน้ำหนัก` หรือรักษาน้ำหนัก | กำหนดเป้าหมายสารอาหารและกลุ่ม analytics |
| `food_allergies` | string | ข้อมูลแพ้อาหาร | แสดงเป็นข้อมูลสุขภาพของผู้ใช้ |
| `avatar_url` | string | URL/พาธรูปโปรไฟล์ | แสดงรูปผู้ใช้ |
| `lifestyle` | string | ระดับกิจกรรม; ค่าเริ่มต้น `light` | คำนวณ TDEE และจัดกลุ่ม analytics |
| `bmi` | float | ดัชนีมวลกายที่ระบบคำนวณ | แสดงสถานะรูปร่าง |
| `bmr` | integer | พลังงานพื้นฐานที่ร่างกายใช้ต่อวัน | แสดงข้อมูลสุขภาพและใช้คำนวณเป้าหมาย |
| `tdee` | integer | พลังงานที่ควรใช้ต่อวันตามกิจกรรม | ใช้เป็นเป้าหมายแคลอรีรายวัน |
| `protein_target` | integer | เป้าหมายโปรตีนต่อวัน | เทียบกับโปรตีนจากมื้ออาหาร |
| `carb_target` | integer | เป้าหมายคาร์โบไฮเดรตต่อวัน | เทียบกับคาร์โบไฮเดรตจากมื้ออาหาร |
| `fat_target` | integer | เป้าหมายไขมันต่อวัน | เทียบกับไขมันจากมื้ออาหาร |

### เมนูอาหารและมื้ออาหาร

| ฟิลด์ | ชนิดข้อมูล | เก็บข้อมูล | ใช้งาน |
| --- | --- | --- | --- |
| `menu_id` | integer | รหัสเมนูอาหารที่มื้ออาหารอ้างอิง | เชื่อมบันทึกมื้อกับเมนูอาหาร |
| `food_name` | string | ชื่ออาหารภาษาไทยในเมนู | ค้นหาและเลือกอาหาร |
| `food_name_en` | string, optional | ชื่ออาหารภาษาอังกฤษ | ค้นหาและแสดงชื่อภาษาอังกฤษ |
| `name` | string | ชื่ออาหารที่บันทึกในมื้อนั้น | แสดงในประวัติและรายละเอียดมื้ออาหาร |
| `calories` | float | พลังงานของอาหาร (กิโลแคลอรี) | รวมยอดและเทียบกับ `tdee` |
| `protein` | float | โปรตีนของอาหาร (กรัม) | รวมยอดและเทียบกับ `protein_target` |
| `carb` | float | คาร์โบไฮเดรตของอาหาร (กรัม) | รวมยอดและเทียบกับ `carb_target` |
| `fat` | float | ไขมันของอาหาร (กรัม) | รวมยอดและเทียบกับ `fat_target` |
| `meal_time` | string | ช่วงเวลาของมื้อ เช่น `breakfast`, `lunch`, `dinner`, `snack` | จัดกลุ่มและแสดงมื้ออาหารในแต่ละวัน |
| `image_url` | string, optional | URL/พาธรูปอาหาร | แสดงรูปในรายการและรายละเอียดมื้ออาหาร |
| `created_at` | datetime | วันที่และเวลาที่บันทึกมื้อ | เรียงประวัติและกรองข้อมูลตามวัน |

### รูปภาพและผล YOLO

| ฟิลด์ | ชนิดข้อมูล | เก็บข้อมูล | ใช้งาน |
| --- | --- | --- | --- |
| `file` | image file | ไฟล์รูปที่ส่งแบบ `multipart/form-data` | ใช้อัปโหลดหรือให้ YOLO วิเคราะห์ |
| `url` | string | พาธรูปที่อัปโหลดแล้ว เช่น `/uploads/abc.jpg` | นำไปแสดงหรือใส่ใน `image_url` ของมื้ออาหาร |
| `filename` | string | ชื่อไฟล์ที่ระบบสร้าง | อ้างอิงไฟล์ที่อัปโหลด |
| `success` | boolean | สถานะการวิเคราะห์ YOLO | แจ้งผลสำเร็จ/ล้มเหลวในหน้าแอป |
| `confidence` | float | ความมั่นใจของผลทำนายอันดับหนึ่ง (0–1) | แสดงเปอร์เซ็นต์ความมั่นใจ |
| `detections` | array | ผลทำนาย 5 อันดับแรก; แต่ละรายการมี `cls`, `label`, `conf` | ให้ผู้ใช้เลือกหรือยืนยันชื่ออาหาร |
| `uploaded_url` | string | พาธรูปต้นฉบับที่อัปโหลด | แสดงรูปต้นฉบับ |
| `original_width` / `original_height` | integer | ขนาดรูปต้นฉบับเป็นพิกเซล | ใช้เมื่อต้องจัดวางหรือแสดงข้อมูลรูป |
